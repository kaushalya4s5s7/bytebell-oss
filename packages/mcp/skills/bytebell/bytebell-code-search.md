...
```
smart_search → (optional) keyword_lookup → retrieve_file metadata → retrieve_file content
```

1. **`smart_search`** — start every investigation here. One query against all six channels in parallel (purpose, paths, keywords, classes, functions, imports). Returns a fused, ranked list of files plus folder clusters. Omit `knowledgeId` for cross-repo search; pass `path: "packages/..."` to narrow to a monorepo subtree; pass `exclude: ["tests", "vendor"]` to drop noise.
...
- Do not guess a `knowledgeId` from a repo name — it is always a UUID.
