...
// Defensive cleanup: wipe File nodes whose knowledgeId has no matching :Knowledge.
const DELETE_ORPHAN_FILES = `
MATCH (f:File)
WHERE NOT EXISTS { MATCH (k:Knowledge {knowledgeId: f.knowledgeId}) }
DETACH DELETE f
`;

// Sweep orphan entities individually.
const DELETE_ORPHAN_KEYWORDS = `
MATCH (n:Keyword)
WHERE NOT EXISTS { MATCH (:File)-[]->(n) } AND NOT EXISTS { MATCH (:Folder)-[]->(n) } AND NOT EXISTS { MATCH (:Repo)-[]->(n) }
DETACH DELETE n
`;

const DELETE_ORPHAN_CLASSES = `
MATCH (n:Class)
WHERE NOT EXISTS { MATCH (:File)-[]->(n) }
DETACH DELETE n
`;

const DELETE_ORPHAN_FUNCTIONS = `
MATCH (n:Function)
WHERE NOT EXISTS { MATCH (:File)-[]->(n) }
DETACH DELETE n
`;

const DELETE_ORPHAN_MODULES = `
MATCH (n:Module)
WHERE NOT EXISTS { MATCH (:File)-[]->(n) }
DETACH DELETE n
`;
...
export async function deleteKnowledgeGraph(knowledgeId: string): Promise<void> {
  ...
  await _runCypher(DELETE_ORPHAN_MODULES);
}
